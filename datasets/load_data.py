...
def load_data(dataset, data_path, representation, normalization, siamese=False):
    if dataset == 'letters':
        ...
        if siamese:
            return load_GREC_siamese(data_path, representation, normalization)
        else:
            return load_GREC(data_path, representation, normalization)
    elif dataset == 'Comic':
        if siamese:
            return load_ICDA_siamese(data_path, representation, normalization)
        else:
            return load_ICDA(data_path, representation, normalization)

    raise NameError(dataset + ' not implemented!')


def load_ICDA(data_path, representation, normalization):
    # Get data for train, validation and test
    data_train = datasets.Comic(data_path, 'train.txt', representation, normalization)
    data_valid = datasets.Comic(data_path, 'valid.txt', representation, normalization)
    data_test = datasets.Comic(data_path, 'test.txt', representation, normalization)

    return data_train, data_valid, data_test

def load_ICDA_siamese(data_path, representation, normalization):
    pass
...
def load_letters(data_path, representation='adj', normalization=False):
    ...
    data_valid = datasets.Letters(data_path, 'valid.cxl', representation, normalization)
...
def load_letters_siamese(data_path, representation='adj', normalization=False):
    ...
    data_valid = datasets.LettersSiamese(data_path, 'valid.cxl', representation, normalization)
...
def load_histograph_siamese(data_path, representation='adj', normalization=False):
    ...
    return data_train, data_valid, data_test
def load_GREC(data_path, representation='adj', normalization=False):
    # Get data for train, validation and test
    data_train = datasets.HistoGraph(data_path, 'train.cxl', representation, normalization)
    data_valid = datasets.HistoGraph(data_path, 'valid.cxl', representation, normalization)
    data_test = datasets.HistoGraph(data_path, 'test.cxl', representation, normalization)

    return data_train, data_valid, data_test


def load_GREC_siamese(data_path, representation='adj', normalization=False):
    # Get data for train, validation and test
    data_train = datasets.HistoGraphSiamese(data_path, 'train.cxl', representation, normalization)
    data_valid = datasets.HistoGraphSiamese(data_path, 'valid.cxl', representation, normalization)
    data_test = datasets.HistoGraphSiamese(data_path, 'test.cxl', representation, normalization)
