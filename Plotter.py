...
def summarize_diagnostics(losses_train, losses_valid, accs_valid, outname):
    # plot loss
    pyplot.subplot(211)
    pyplot.title('Cross Entropy Loss')
    pyplot.plot(losses_train, color='blue', label='train')
    pyplot.plot(losses_valid, color='red', label='test')
    plt.savefig(outname)

    # plot accuracy
    pyplot.subplot(212)
    pyplot.title('Classification Accuracy')
    pyplot.plot(accs_valid, color='red', label='test')
    plt.savefig(outname)
