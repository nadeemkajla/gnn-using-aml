...
from Plotter import summarize_diagnostics
...
from utils import save_checkpoint, load_checkpoint, accuracy, knn, write_gxl, confusion_matrix, build_confusion_matrix, \
    roc_auc
import models
...
def validation(test_loader, net, cuda, criterion, evaluation, compute_confusion_matrix=False):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    acc = AverageMeter()
    ...
    if compute_confusion_matrix:
        build_confusion_matrix(outputs, targets)
        roc_auc(outputs, targets)
...
def main():
    ...
    if args.representation=='adj':
        ...
        net = models.MpnnGGNN(in_size=data_train.input_size, e=[1], hidden_state_size=args.hidden_size, message_size=args.hidden_size, n_layers=args.nlayers, discrete_edge=True, target_size=data_train.getTargetSize())
    elif args.representation=='feat':
        print('\t* Feature Edges')
        net = models.MpnnGGNN(in_size=data_train.input_size, e=2, hidden_state_size=args.hidden_size, message_size=args.hidden_size, n_layers=args.nlayers, discrete_edge=False, target_size=data_train.getTargetSize())
    else:
    ...
    elif args.distance=='Heomd':
        print('\t* Heterogeneous Euclidean Overlap Metric distance')
        distance = GraphEditDistance.Heomd()
    else:
    ...
    if not args.test:
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            losses_train.append(loss_train.avg)
            losses_valid.append(loss_valid.avg)
            accs_valid.append(acc_valid.avg)

            logger.step()
        summarize_diagnostics(losses_train, losses_valid, accs_valid, 'learn_accuracy.png')
    ...
    print('Test:')
    loss_test, acc_test = validation(test_loader, net, args.ngpu > 0, criterion, evaluation, True)
    print('Test '+ args.distance + ' distance:')
    test_loader = torch.utils.data.DataLoader(data_test,
